...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error
{
    ...
    for (CBCharacteristic *charac in service.characteristics)
    {
        if ([charac.UUID isEqual:BLTUUID.rxCharacteristicUUID])
        {
            ...
            {
                _updateModelBlock(nil, BLTUpdateModelDidConnect);
            }
...
- (void)peripheral:(CBPeripheral *)peripheral didReadRSSI:(NSNumber *)RSSI error:(NSError *)error
{
    if (_updateModelBlock)
    {
        ...
        [BLTManager sharedInstance].model.bltRSSI = [NSString stringWithFormat:@"%d", (int)rssi];
        _updateModelBlock([BLTManager sharedInstance].model, BLTUpdateModelRSSI);
    }
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    else
    {
        ...
        {
            SHOWMBProgressHUD(@"蓝牙服务连接失败", nil, nil, NO, 2.0);
            [[BLTManager sharedInstance] dismissLinkAndRepeatConnect];
            return;
        }
         NSLog(@"打印服务>>>>..%@..%@", characteristic.value, characteristic.UUID);
        if ([characteristic.UUID isEqual:BLTUUID.rxCharacteristicUUID] ||
            [characteristic.UUID isEqual:BLTUUID.bigDataReadCharacteristicUUID])
        {
            ...
            if (_updateDataBlock)
            {
                _updateDataBlock(_receiveData, BLTUpdateDataTypeNormalData,peripheral);
            }
        }
...
//    if (_updateDataBlock)
//    {
//        _updateDataBlock(nil, BLTUpdateDataTypeTransFail);
//    }
}
