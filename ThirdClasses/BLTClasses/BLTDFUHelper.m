...
{
    if (_isFoundControlPointChar && _isFoundPacketChar && _isFoundVersionChar && _isUpdateMode)
    {
        [self performDFUOnFileWithMetaData];
    }
}

- (void)gotoUpdateState
{
...
-(void)performDFUOnFileWithMetaData
{
    ...
    NSLog(@"performDFUOnFileWithMetaData array >>>%@",array);
...
- (void)delaySendDFu
{
    NSLog(@"delaySendDFu>>>>>>>>delaySendDFu");
...
- (void)writeFileSize:(uint32_t)firmwareSize
{
    ...
    NSLog(@"fileRequests.binFileSize = %d", firmwareSize);
...
- (void)onReadDfuVersion:(NSInteger)version
{
    ...
    NSLog(@"onReadDfuVersion version = %ld", (long)version);
    if (version == 0) {
        [self resetSystem];
    } else {
        if (version == 1) {
