...
- (instancetype)init
{
    ...
    if (self)
    {
        ...
        _backgroundMode = BackgroundModeUnknown;
        
        _sportDetailArr = [NSMutableArray array];
        _sleepDetailArr = [NSMutableArray array];
    }
...
- (SportModel *)todaySport
{
    ...
    if (!_todaySport || ![_todaySport.date isEqualToString:dateStr]) {
...
- (SportModel *)yesSport
{
    if (!_yesSport) {
        ...
        _yesSport.date = [[[NSDate date] dateAfterDay:-1] dateToDayString];
    }
...
{
    NSString *dateStr = [[NSDate date] dateToDayString];
    if (!_todaySleep || ![_todaySleep.date isEqualToString:dateStr]) {
...
- (SleepModel *)yesSleep
{
    if (!_yesSleep) {
        ...
        _yesSleep.date = [[[NSDate date] dateAfterDay:-1] dateToDayString];
    }
